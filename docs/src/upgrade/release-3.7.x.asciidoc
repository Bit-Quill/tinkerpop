...
Previously, we need to use the Neo4j gremlin plugin to perform transactions in TinkerGraph. Unfortunately, the maintenance
around this plugin has largely been abandoned and is only compatible with Neo4j version 3.4, which reached end of
life in March 2020.
...
Neo4j gremlin plugin. It only imposes write-only locks, to reduce the impact of transactions on performance for reading. As well,
only ThreadLocal transaction is implemented, therefore embedded graph transactions may not be fully supported.
