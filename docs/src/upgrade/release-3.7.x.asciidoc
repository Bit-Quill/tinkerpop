...
Neo4jGraph provided in `neo4j-gremlin`. It only imposes write-only locks, to reduce the impact of transactions on
performance for reading. As well, only ThreadLocal transactions are implemented, therefore embedded graph transactions
may not be fully supported. You can think of the transaction as belonging to a thread, so any traversals executed within
the same thread will share the same transaction even if you attempted to start a new transaction.
...
will throw a `TransactionException` if two different transactions attempt to lock the same element. Users should follow
...
exponential backoff based retries to mitigate this issue. As mentioned before, TinkerTransactionGraph has
`read committed` isolation level, so it only guards against dirty reads. These transaction semantics may not fit the use
case for many production scenarios that require strict ACID-like transactions. Therefore, it is recommended that
TinkerTransactionGraphbe used as a Graph for test environments where you still require access to a Graph that supports
transactions. Once you are ready to put your system into production then you can choose a Graph provider that has the
...
Note that all embedded TinkerTransactionGraph remains ThreadLocal transactions, meaning that all traversal sources spawn
from the graph will operate within the same transaction scope.

===== Limitations
At this point, the transactional TinkerGraph comes with performance limitations and thus is only meant for testing
