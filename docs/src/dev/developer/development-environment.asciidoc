...
The following are the steps a Windows user must take in order to Build Tinkerpop on Windows:
...
NOTE: The Windows build of Tinkerpop has currently only been tested to stably work on v3.5. Any other versions of Tinkerpop on Windows may still build, but to a partial extent.
