...
`gremlin-dotnet` uses Docker for running integration tests against the Gremlin server. Please make sure Docker is installed and running on your system. 
...
`gremlin-javascript` uses Docker for running integration tests against the Gremlin server. Please make sure Docker is installed and running on your system. 
You will need to install both link:https://docs.docker.com/engine/install/[Docker Engine] and link:https://docs.docker.com/compose/install/[Docker Compose], which are included in link:https://docs.docker.com/desktop/[Docker Desktop].
...
Some GLV allow you to run tests without Gremlin Server build. Currently supported gremlin-go, gremlin-javascript and
gremlin-dotnet.
...
set up. This varibale is set by default in Unix/Linux and macOS, but will need to be set for Windows, for example, run 
